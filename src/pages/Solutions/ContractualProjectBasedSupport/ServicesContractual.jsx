  ...
  return (
    <div className={styles.gridContainer}>
      {serviceData.map((item) => (
        ...
        >
          <div className={`${styles.cardInner} ${hoveredId === item.id ? styles.showDetails : ''}`}>
            {/* Front - Card */}
            ...
            <div className={styles.cardBack}>
              ...
              {Array.isArray(item.details) && item.details[0]?.items ? (
                item.details.map((group, i) => (
                  <div key={i}>
                    <h4 className={styles.groupHeading}>{group.heading}</h4>
                    <ul className={styles.detailsList}>
                      {group.items.map((detail, j) => (
                        <li key={j}>{detail}</li>
                      ))}
                    </ul>
                  </div>
